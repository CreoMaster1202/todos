$(document).ready(function() {
    $('#add-list-form').on('submit', function(event) {
      ...
      $.post('/add_list', {name: name}, function(data) {
        var newList = $('<li data-id="' + data.id + '"><h2>' + name + '</h2><div class="editcard" style="display: none;"><form class="edit-list-form"><textarea name="name">' + name + '</textarea><button type="submit">Save</button></form><form class="delete-list-form"><button type="submit">Delete</button></form></div><ul class="items"></ul><form class="add-item-form"><textarea name="description" placeholder="Item description"></textarea><button type="submit">Add Item</button></form></li>');
